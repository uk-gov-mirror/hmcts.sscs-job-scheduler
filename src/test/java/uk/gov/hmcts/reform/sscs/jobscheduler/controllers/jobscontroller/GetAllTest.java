package uk.gov.hmcts.reform.sscs.jobscheduler.controllers.jobscontroller;
...
import uk.gov.hmcts.reform.sscs.jobscheduler.FixtureData;
import uk.gov.hmcts.reform.sscs.jobscheduler.SampleData;
import uk.gov.hmcts.reform.sscs.jobscheduler.controllers.JobsController;
import uk.gov.hmcts.reform.sscs.jobscheduler.model.JobData;
import uk.gov.hmcts.reform.sscs.jobscheduler.model.JobSchedulerPageRequest;
import uk.gov.hmcts.reform.sscs.jobscheduler.model.Pages;
import uk.gov.hmcts.reform.sscs.jobscheduler.services.jobs.JobsService;
...
public class GetAllTest {
    ...
    public void should_return_default_pages_when_no_parameters_are_passed() throws Exception {
        JobData jobData = new JobData("some-id", SampleData.validJob());
        when(jobsService.getAll(anyString(), anyInt(), anyInt()))
    ...
    private ResultActions getResponse(int page, int size) throws Exception {
        ...
        if (copies < 1) {
            ...
            jobs = Collections.nCopies(Math.min(copies, size), new JobData("some-id", SampleData.validJob()));
        }
